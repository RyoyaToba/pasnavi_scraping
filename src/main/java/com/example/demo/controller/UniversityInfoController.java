...
public class UniversityInfoController {
	...
	public static void main(String[] args) throws IOException {
		...
		int listNumber = idList.indexOf(1362);
		
		for (int i = listNumber; i < idList.size(); i++) {
			...
			for (Element element : elements) {
				if (element.text().contains("|")) {
					...
					university.setGenderCateggoryId(
							ConversionGenderCategory.conversionCategory(createFemailCategory(elements)));
				} else {
					university.setUniversityCategoryId(ConversionUniversityCategory.conversionCategory("共学"));
					university.setGenderCateggoryId(ConversionGenderCategory.conversionCategory(element.text()));
				}
			}
			...
			try {
				sql = "INSERT INTO university (id, name, university_category_id, prefecture_id, gender_category_id) "
					+ "VALUES (" + university.getId() + ","  
								...
								+ university.getGenderCateggoryId() + ")";
